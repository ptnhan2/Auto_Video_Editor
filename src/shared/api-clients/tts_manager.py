...
class TTSManager:
    ...
    async def render(self, text, provider, voice, filename, **kwargs):
        ...
        # Số lần thử tối đa: Thử mỗi proxy ít nhất 2 lần để vượt qua lỗi vượt giới hạn (rate limit) tạm thời
        proxy_count = len(self.proxies)
        max_attempts = max(proxy_count * 2, 5) if proxy_count else 3
        last_error = ""

        for attempt in range(max_attempts):
            # Chọn proxy mới cho mỗi lần thử (nếu có)
            current_proxy = None
            if self.proxies:
            ...
            try:
                if provider == "edge":
                    ...
                    try:
                        ...
                        success, result = await run_edge(text, voice, filepath, **kwargs)
                    ...
                    success, result = await asyncio.to_thread(run_tiktok, text, voice, session, filepath)
                    ...
                    # Chạy trong thread để không block event loop
                    success, result = await asyncio.to_thread(run_elevenlabs, text, voice, model, settings, latency, filepath, self.elevenlabs_keys)
                ...
                # Log chi tiết lỗi và proxy đang dùng để dễ debug
                proxy_info = f" (Proxy: {current_proxy})" if current_proxy else ""
                print(f"   ⚠️ Lần {attempt + 1}/{max_attempts} thất bại cho '{filename}': {last_error}{proxy_info}. Đang thử lại...")
                
                if attempt < max_attempts - 1:
                    # Nếu đã thử qua một lượt tất cả proxy, hãy nghỉ lâu hơn một chút để reset rate limit
                    wait_time = 1.0 if proxy_count and (attempt + 1) % proxy_count == 0 else 0.5
                    await asyncio.sleep(wait_time)
                    
            except Exception as e:
                last_error = str(e)
                print(f"   ❌ Ngoại lệ tại lần thử {attempt + 1}: {last_error}")
                if attempt < max_attempts - 1:
                    await asyncio.sleep(0.5)

        return False, f"Thất bại sau {max_attempts} lần thử. Lỗi cuối: {last_error}"
