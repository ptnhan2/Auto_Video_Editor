  ...
  if (fs.existsSync(backgroundDir)) {
    ...
    bgFiles.forEach(file => {
      ...
      if (['.jpg', '.png', '.jpeg', '.webp'].includes(ext)) {
         const id = path.basename(file, ext); // No more mandatory 'bg_' prefix
...
`;
  ...
  // ALSO OUTPUT JSON FOR PYTHON PIPELINE
  const jsonOutput = {
    characters,
    backgrounds,
    audioTracks,
  ...
  const JSON_FILE = path.join(process.cwd(), 'public', 'asset_registry.json');
  fs.writeFileSync(JSON_FILE, JSON.stringify(jsonOutput, null, 2), 'utf-8');
  console.log('✅ Successfully synced JSON registry at public/asset_registry.json');
}
